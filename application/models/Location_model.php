...
public function contact($lead_code) {
    ...
    public function get_city_byidqr($id5) {
    ...
    public function estate_list() {
     ...
     public function estate_list_qr($compid=0) {
        if ($compid == 0) {
            $compid    =   $this->session->companey_id;
        }
        return $this->db->select("*")
                        ->from("state")
                        ->where('state.comp_id', $compid)
                        ->get()
                        ->result();
    }
    ...
    public function ecity_list_qr($compid=0) {
        if ($compid == 0) {
            $compid    =   $this->session->companey_id;
        }
        return $this->db->select("*")
                        ->from("city")
                        ->where('city.comp_id', $compid)
                        ->order_by('city','asc')
                        ->get()
                        ->result();
    }
    ...
    public function ecountry_list_qr($compid=0) {
        if ($compid == 0) {
            $compid    =   $this->session->companey_id;
        }
        return $this->db->select("*")
...
/***---------------------start api--------------------------***/
/*************************************payment list *******************************/
public function get_fee_list($process_id=0){
    $this->db->select("*");
   ...
   /*************************************payment list End*******************************/
   /*************************************aggriment list *******************************/
   public function get_client_form($enq_no,$select_key){
        if (!empty($enq_no)) {                        
            $this->db->select('key_parameter as parameter,key_value as value,key_type');
            $this->db->where('enq_id',$enq_no);
			$this->db->where('key_type',$select_key);
            $result    =   $this->db->get('tbl_aggriment')->result_array();
            $data = array();
            if (!empty($result)) {
...
public function get_same_data_email($smae_id)
    {
        return $this->db->select('*')
                        ->from('enquiry')
                        ->where('email',$smae_id)
...
///////////////// Agreement Format SECTION START////////////////////
public function agrformat_select($enq_id) {
        $this->db->select("*,tbl_agreement_template.template_name as tname,tbl_client_agreement.id");
        ...
        $this->db->where('tbl_client_agreement.comp_id', $this->session->userdata('companey_id'));
		$this->db->where('tbl_client_agreement.enq_id', $enq_id);
        $query = $this->db->get();
        return $query->result();
    }
	
    public function agreement_format() {
        $data['nav1'] = 'nav2';
        if (!empty($_POST)) {
...
public function delete_template(){
    ...
    ///////////////// Agreement Format SECTION END////////////////////
    #-----------------------------------------file manager start-------------------------------------------------# 
    public function file_manager_add($data) {
        $this->db->insert('tbl_filemanager', $data);
    }
    public function file_select() {
        $this->db->select("*");
        $this->db->from('tbl_filemanager');
        $this->db->where('comp_id', $this->session->userdata('companey_id'));
        $query = $this->db->get();
        return $query->result();
    }
...
#-----------------------------------------All document Master setup Start-------------------------------------------------#	
	public function country_vs_document_add($data) {
        $this->db->insert('tbl_doc_vs_country', $data);
    }
	public function c_vs_d_select() {
        $this->db->select("*");
	...
	/*****************************************************************************/
	public function country_vs_stream_add($data) {
        $this->db->insert('tbl_doc_vs_sream', $data);
    }
	public function c_vs_s_select() {
        $this->db->select("*");
	...
	/****************************************************************************/
	public function country_vs_file_add($data) {
        $this->db->insert('tbl_doc_vs_file', $data);
    }
	public function c_vs_f_select() {
        $this->db->select("*");
...
/*************************************Refund list *******************************/
public function get_refund_list($process_id=0){
    $this->db->select("*");
    $this->db->from('tbl_refund');
    $this->db->where('enquiry_id',$process_id);
    $this->db->order_by('id', 'asc');
    return $this->db->get()->result();
   }

   public function get_ticket_list($process_id=0){
    
    $this->db->select("resp_id");
    $this->db->from('query_response');
    if($this->session->userdata('user_right')!=214){
        $this->db->where('create_by',$this->session->userdata('user_id'));
    }else{
        $this->db->where('related_to',$this->session->userdata('user_id'));
    }
    $this->db->where('subject','Agreement Ticket');
    $this->db->where('noti_read','0');
    return $this->db->get()->result();
   }
/*************************************Refund list End*******************************/
/*************************************Urls list Start*******************************/
public function get_url_list($country_id=0){
    $this->db->select("*");
    $this->db->from('tbl_url');
    $this->db->where('url_counrty',$country_id);
