...
public function all_course($course,$lvl,$length,$disc) {
    ...
    public function get_leadListDetailsby_id($leadid) {
        return $this->db->select("*,enquiry.created_date,tbl_newdeal.bank,enquiry.country_id as enq_country,enquiry.state_id as enquiry_state_id,enquiry.city_id as enquiry_city_id,enquiry.address,enquiry.status,tbl_product_country.country_name,tbl_product_country.id as country_id,tbl_product.product_name,tbl_center.center_name,tbl_installment.Pay_status as ins_pay,tbl_payment.status as full_pay")
                        ->from('enquiry')
                        ...
                        ->join('tbl_installment','tbl_installment.enq_id=enquiry.Enquery_id','left')
                        ->join('tbl_payment','tbl_payment.enq_id=enquiry.Enquery_id','left')
...
function find_stage($key='') {
    ...
    $department = $this->session->userdata('dept_name');
if(count($process_filter1)>'1'){
    $where = "FIND_IN_SET('".$key."', stage_for)";
    $where_dept = "FIND_IN_SET('".$department."', departments_for)";
       
      
        $this->db->select(" * ");
        $this->db->from('lead_stage');
        $this->db->where('comp_id',$this->session->userdata('companey_id'));
        $this->db->where('stage_for_disp','2');
        $this->db->where( $where );
        $this->db->where( $where_dept );
...
function get_leadstage_name($id) {
    ...
    function find_estage($product_id,$stage_for='') {
        ...
        if ($department) {
            $this->db->where("FIND_IN_SET('".$department."', departments_for)>",0);
        }
            $this->db->order_by("stage_order", "asc");
            $query = $this->db->get();            
            $res = $query->result();
     ...
     function find_cstage($product_id,$stage_for='') {
        if (!empty($product_id)) {
            $department = $this->session->userdata('dept_name');

            $this->db->select("*");
            $this->db->from('lead_stage');
            $this->db->where('stage_for_disp','1');            
            $this->db->where("FIND_IN_SET($product_id,lead_stage.process_id)>",0);
            if ($stage_for) {
                $this->db->where("FIND_IN_SET($stage_for,lead_stage.stage_for)>",0);                
            }
    ...
    function find_description($product_id,$stage_for='') {
        ...
        $query = $this->db->get();            
        $res = $query->result();
        $stage_id = array_column($res, 'stg_id');
        
        $this->db->select("*");
        $this->db->from('lead_description');
        foreach ($stage_id as $key => $value) {
           $this->db->or_where("FIND_IN_SET($value,lead_stage_id)>",0);
        }
        $this->db->where('comp_id', $this->session->userdata('companey_id'));
        $querys = $this->db->get();
...
public function all_class_list($diesc,$diesc1) {
  ...
  public function get_leadsource_list_api($compno) {
    ...
    public function get_desptype_list() {
        $this->db->select('*');        
        $this->db->where('comp_id',$this->session->userdata('companey_id'));
        $this->db->order_by("id", "asc");
        $query = $this->db->get('desposition_type');
        return $query->result();
    }
    ...
    public function desp_typeadd($data) {
        $this->db->insert('desposition_type', $data);
    }
    ...
    public function delete_type($type = null) {
        $this->db->where('id', $type)
                ->delete('desposition_type');
    ...
    public function comment_byId($lead_code,$type='4') {
        $this->db->select('*,tbl_comment.mail_header as msub,tbl_comment.remark as rmk,tbl_comment.status as st,tbl_comment.created_date as ddate,tbl_admin.s_display_name as comment_created_by,tbl_admin.last_name as lastname,tbl_user_role.user_role as role_name,desposition_type.type_name,call_stg.lead_stage_name as call_stage,lead_stage.lead_stage_name as stage_name');
        $this->db->from('tbl_comment');
        $this->db->where('lead_id', $lead_code);
        $this->db->where('msg_type', $type);
        $this->db->join('desposition_type','desposition_type.id=tbl_comment.ctype_disposition','left');
        ...
        $this->db->join('lead_stage as call_stg','call_stg.stg_id=tbl_comment.call_stage_id','left');
    ...
    public function comment_byId_applicant($lead_code,$type='4') {
        ...
        $this->db->select('*,tbl_comment.mail_header as msub,tbl_comment.remark as rmk,tbl_comment.status as st,stage_visible.lang_name as lead_stage_name,tbl_comment.created_date as ddate,tbl_admin.s_display_name as comment_created_by,tbl_admin.last_name as lastname,tbl_user_role.user_role as role_name,desposition_type.type_name');
        $this->db->from('tbl_comment');
        $this->db->where('lead_id', $lead_code);
        $this->db->where('msg_type', $type);
    ...
    public function add_comment_for_events($conversation, $lead_id,$stage_code=0,$user_id=0,$stage_remark='',$type='4',$status='',$mail_header='',$image='') {
        ...
        $this->db->set('mail_header', $mail_header);
        ...
        $this->db->set('msg_type', $type);
        $this->db->set('status', $status);
        $this->db->set('attachment', $image);
    ...
    public function add_comment_for_events1($conversation, $lead_id,$reason,$drop_status,$type='4') {
   ...
   public function add_comment_for_events_stage($conversation, $lead_id,$stage_id,$stage_desc,$stage_remark='',$coment_type,$type='4',$type_disposition='',$call_stage='') {
        $ld_updt_by = $this->session->user_id;
        $adt = date("Y-m-d H:i:s");
        $this->db->set('comp_id',$this->session->userdata('companey_id'));
        $this->db->set('lead_id', $lead_id);
        ...
        $this->db->set('call_stage_id', $call_stage);
        $this->db->set('stage_description', $stage_desc);
        $this->db->set('remark', $stage_remark);
        $this->db->set('msg_type', $type);
        $this->db->set('ctype_disposition', $type_disposition);
        $this->db->insert('tbl_comment');
    }

    public function add_comment_for_events_stage_api($conversation, $lead_id,$stage_id,$stage_desc,$stage_remark,$user_id,$comment_type=0,$type='4') {
    ...
    public function add_comment_for_events_popup($stage_remark,$stage_date,$contact_person,$mobileno,$email,$designation,$stage_time,$enq_code,$notification_id=0,$dis_subject='',$type_disposition='',$call_stage='') {
        $ld_updt_by = $this->session->user_id;
        $adt = date("Y-m-d H:i:s");
    ...
    function find_stage_api($comp) {
     ...
     public function select_branch_by_dept_id($dept) {
        ...
        $this->db->where('branch_dept',$dept);
        $query1 = $this->db->get()->row();

        $branch_ids = array();
        $branch_ids    =   explode(',', $query1->branch_name);
   ...
   /******************************************************End api*****************************************************/
    ...
    public function ticket_subject_add($data) {
        $this->db->insert('tbl_ticket_subject', $data);
    }

    public function ticket_subject_select() {
        $this->db->select("*");
        $this->db->from('tbl_ticket_subject');
        $this->db->where('comp_id', $this->session->userdata('companey_id'));
        $query = $this->db->get();
        return $query->result();
    }

    public function delete_ticket_subject($ticket_subject_id = null) {
        $this->db->where('id', $ticket_subject_id)->delete('tbl_ticket_subject');
        if ($this->db->affected_rows()) {
            return true;
        } else {
            return false;
...
public function visa_type_select_qr($compid=0) {
    if ($compid == 0) {
        ...
        if(is_array($for))
        {
            foreach ($for as $stage_for)
            {
                $this->db->select('des.id,des.description,stg.stage_for,stg.process_id')
                            ...
                            ->where('des.comp_id',$this->session->companey_id)
