...
public function __construct()
    ...
    public function search_taskby_filter($from='',$to='',$task_status='',$user_mobile='',$user_name='') {
...
//TASK FILTER CODE START HERE            
            if ($from && $to) {
                ...
                $where .= "AND STR_TO_DATE(query_response.task_date,'%d-%m-%Y') >= '$from' AND STR_TO_DATE(query_response.task_date,'%d-%m-%Y') <= '$to' ";
            } else if ($from && !$to) {
                $from = str_replace('/', '-', $from);            
                $from = date('Y-m-d', strtotime($from));
                $where .= "AND STR_TO_DATE(query_response.task_date,'%d-%m-%Y') LIKE '%$from%' ";
            } else if (!$from && $to) {            
                $to = str_replace('/', '-', $to);
                $to = date('Y-m-d', strtotime($to));
                $where .= "AND STR_TO_DATE(query_response.task_date,'%d-%m-%Y') LIKE '%$to%' ";
            }
            ...
            if ($task_status=='1') { 
                ...
                $where .= "AND query_response.task_status = '2' ";
            }

            if (!empty($user_mobile)) {            
                $where .= "AND enquiry.phone LIKE '%$user_mobile%' ";
            }

            if (!empty($user_name)) {            
                $where .= "AND enquiry.name LIKE '%$user_name%' OR enquiry.lastname LIKE '%$user_name%' ";
            }
