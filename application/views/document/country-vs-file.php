         ...
         <div class="panel-body">
               <div class="tab-content clearfix">                  
                  <div class="tab-pane active" id="tab-templates">
                        ...
                        <table width="100%" class="datatable table table-striped table-bordered table-hover">
                           <thead>
                              <tr>
								 ...
								 <th  nowrap>Document File Name</th>
                              </tr>
                           </thead>
                           <tbody>
                              <?php if (!empty($all_c_vs_f)) { ?>
                              <?php $sl = 1; ?>
                              <?php foreach ($all_c_vs_f as $cvf) { ?>
                              <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>" style="cursor:pointer;" data-toggle="modal" data-target="#createnewtemplate<?php echo $cvf->id;?>">
                                 <td class="th0"><input type='checkbox' onclick="event.stopPropagation();" name='sel_temp[]' class="checkboxtemp" value='<?php echo $cvf->id;?>'>&nbsp; <?php echo $sl;?></td>
                                 <td class="th1"><?php foreach($country as $cntry){ if($cntry->id_c==$cvf->country_id){echo $cntry->country_name;}} ?></td>
								 <td class="th1"><?php foreach($all_c_vs_d as $cvd){ if($cvd->id==$cvf->document_type){echo $cvd->document_type;}} ?></td>
								 <td class="th1"><?php foreach($all_c_vs_s as $cvs){ if($cvs->id==$cvf->stream){echo $cvs->stream;}} ?></td>
								 <td class="th1"><?php echo $cvf->file_name; ?></td>
                              </tr>
                              <?php $sl++; ?>
                              <?php } ?> 
                              <?php } ?> 
                     ...
                     <?php foreach ($all_c_vs_f as $tlist) { ?>
                        ...
                        <div class="modal-dialog modal-lg">
                           ...
                           <div class="modal-content" >
                              ...
                              <div class="modal-body">
                                 <!--<form>-->
                                 <?php echo form_open_multipart('location/cvf_update','class="form-inner"') ?>                      
                                 <div class="row">
                                    <input type="hidden" name="cvf_id" value="<?php echo $tlist->id; ?>">
                                    <div class="form-group   col-sm-6">
                                       <label>Preferd Country</label>
                                       <select class="form-control preferd_country_edit"  name="preferd_country"  id="preferd_country" data-id="<?= $tlist->id; ?>"> 
                                    ...
                                    <div class="form-group   col-sm-6">
                                      ...
                                      <select class="form-control document_type_edit " data-id="<?=  $tlist->id;  ?>" name="document_type"  id="document_type_<?= $tlist->id; ?>" > 
									...
									<div class="form-group   col-sm-6">
                                      ...
                                      <select class="form-control"  name="stream"  id="steam_<?= $tlist->id; ?>"> 
                                           <option>Select Stream Name</option>
					                    <?php foreach($all_c_vs_s as $cvs){ ?>
                                           <option value = "<?php echo $cvs->id; ?>" <?php if($cvs->id==$tlist->stream){ echo 'selected';};?>><?php echo $cvs->stream; ?></option>
					                    <?php } ?>
                                       </select>
                                    </div>
									<div class="form-group   col-sm-6">
                                      <label>Document File Name*</label>
                                      <input class="form-control" name="file_name" type="text" required="" value="<?php echo $tlist->file_name; ?>">
                                    </div>
                                 </div>
...
<div id="createnewtemplate" class="modal fade" role="dialog">
   <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content">
         ...
         <div class="modal-body">
            <!--<form>-->
            <?php echo form_open_multipart('location/country_vs_file','class="form-inner" id="country_vs_stream"') ?>                      
            <div class="row">
			    <div class="form-group   col-sm-6">
                  <label>Preferd Country</label>
                ...
                <div class="form-group   col-sm-6">
					<label>Document Stream Name*</label>
                        <select class="form-control stream"  name="stream"  id="stream" > 
                            <option>Select Stream Name</option>
				...
				<div class="form-group   col-sm-6">
                    <label>Document File Name*</label>
                    <input class="form-control" name="file_name" type="text" required="" value="">
                </div>
            </div>
...
<script>   
   ...
   url: '<?php echo base_url();?>location/delete_cvf',
...
var id = $(this).val();
 $.ajax({
   type: 'POST',
   data:{id:id},
   url: '<?php echo base_url();?>location/fetch_stream',
   success: function(data)
       {
      $(".stream").html(data);
       }
...
// var ids = $("#preferd_country_edit"+id).val();
 $.ajax({
   ...
   $('.document_type_edit').change(function(){ 
var id = $(this).val();
 var x=$(this).data('id');
// var ids = $("#preferd_country_edit"+id).val();
 $.ajax({
