                     ...
                     <table class="table table-striped table-bordered add-data-table" id="filtered_Data1" cellspacing="0" width="100%">
								...
								<?php   
										...
										if(!empty($tags)){
											...
											if($this->session->user_id==$rslt['created_by'] || $rslt['visiblity']=='1'){
											?>
										<tr>	
											...
											<td>
											<?php if($this->session->user_id==$rslt['created_by']){ ?>
												<a href="<?=base_url().'lead/delete_tag/'.$rslt['id']?>" onclick="return confirm('Are You sure?');" class='btn btn-danger btn-xs'><i class="fa fa-trash"></i></a>
											<?php } ?>
											</td>
										</tr>		
								    <?php  }
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
      	<form action="<?=base_url().'lead/tags'?>" method='post'>	      		
	      	...
	      	<?php if($this->session->user_id=='535'){ ?>
	      		<div class="col-md-2">      			
		      		<label>Visible To</label>
		      		<input type="checkbox" class="form-control" value="1" name="visible">
	      		</div>
	      	<?php } ?>
	      	</div>
	      	<div class="row">	      		
