      ...
      <?php if(!empty($this->session->telephony_token)){ ?>
            ...
            <div class="dropdown-menu dropdown_css" style="max-height: 400px;overflow: auto;">
              ...
              <?php
                  ...
                  <a class="btn" data-toggle="modal" data-target="#genrefund" style="color:#000;cursor:pointer;border-radius: 2px;border-bottom :1px solid #fff;">Move To Refund Case </a>
...
<!------------for move to refund case-------------->
<div id="genrefund" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Enter info and Move to Refund Case</h4>
      </div>
      <div class="modal-body">        
          <div class="row">
            <div class="form-group col-sm-6">  
      ...
      <div class="modal-footer">
        <button class="btn btn-success" type="button" onclick="moveto_refund();" >Move to Refund Case</button> 
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
...
function assign_enquiry(){
  if($('.checkbox1:checked').size() > 1000){
    ...
    if (data_type == 1) {
      ...
      var p_url = '<?php echo base_url();?>refund/assign_enquiry';
      var re_url = '<?php echo base_url();?>refund/index';
    }    
...
function moveto_refund(){
  if($('.checkbox1:checked').size() > 1000){
    alert('You can not move more that 1000 Cases at once');
  }else{
  $.ajax({
  type: 'POST',
  url: '<?php echo base_url();?>refund/move_to_refund',
  data: $('#enquery_assing_from').serialize(),
  success:function(data){
      if(data=='1'){
           alert('Successfully Moved in Refund Case'); 
        window.location.href='<?php echo base_url();?>client/index'
      }else{
       alert(data);
      }
  }});
