...
class LeadRules extends CI_Controller {
    ...
    public function save_rule($id=0){                
        ...
        $rule_execution    =   $this->input->post('rule_execution');
    ...
    public function create_rule($id=0){
        ...
        $drop_reason = $this->Leads_Model->get_drop_list();
        $all_tags = $this->Leads_Model->get_tags_list();
        ...
        if (!empty($drop_reason)) {
            foreach ($drop_reason as $key => $value) {
                $rule_drops[$value->d_id]  = $value->drop_reason;
            }
        }
        ...
        if (!empty($all_tags)) {
            foreach ($all_tags as $key => $value) {
                $rule_tags[$value->id]  = $value->title;
            }
        }

        $data['lead_stages'] = $this->Leads_Model->getStageJson(array(1,2,3,4));
        $data['ticket_stages'] = $this->Leads_Model->getStageJson(array(4));
        $data['lead_description'] = $this->Leads_Model->getSubStageJson(array(1,2,3,4));
        ...
        $enq_status_oth = array(1=>'Enquiry',2=>'Lead',3=>'Client',4=>'Refund');
        ...
        $data['user_list']   = $this->user_model->companey_users_rule();
        $data['products']   = json_encode($rule_product);
        $data['sub_source']   = json_encode($rule_subsource);

        $data['drops']   = json_encode($rule_drops);
        $data['tags']   = json_encode($rule_tags);
