      ...
      if(empty($_POST['other_id']))
       ...
       if(is_array($process_id))
        ...
        if(!empty($user_id))
        {
           ...
           $total = $this->enquiry_model->active_enqueries_api($user_id,$data_type_id,$user_role,$process,$filter)->num_rows();
          
            $data['active_enquiry'] = $this->enquiry_model->active_enqueries_api($user_id,$data_type_id,$user_role,$process,$filter,$offset,$limit);
