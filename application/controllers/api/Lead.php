...
class Lead extends REST_Controller {
  ...
  //                   array_push($res,array('enquery_id'=>$value->enquiry_id,'enquery_code'=>$value->Enquery_id,'org_name'=>$value->org_name,'customer_name'=>$value->name_prefix.' '.$value->name.' '.$value->lastname,'email'=>$value->email,'phone'=>$value->phone,'state'=>'','source'=>'test','type'=>$customer));  
  //                } 
  //              }
	// 		}
	// 		}			
  ...
  {   
    ...
    if(!empty($user_id))
    {
      ...
      if(!empty($user_role1)){
      $user_role=$user_role1->user_roles;
      $total = $this->enquiry_model->active_enqueries_api($user_id,2,$user_role,$process,$filter)->num_rows();
      $data['active_enquiry'] = $this->enquiry_model->active_enqueries_api($user_id,2,$user_role,$process,$filter,$offset,$limit);
      if(!empty($data['active_enquiry']->result()))
      {
        $res['offset'] = $offset;
        $res['limit'] = $limit;
...
public function tags_post()
{
  ...
  $this->form_validation->set_rules('title','Tag Title' ,'required');
  ...
  if($this->form_validation->run() == true)
  {
    ...
    if(!empty($update))
    {
      $update_id = $this->db->set($data)->where('id' , $update)->update('tags');
      $msg = 'Tag Updated';
      $status = true;
    }
    else
...
{
  $this->form_validation->set_rules('id','Tag ID' ,'required');
  $id = $this->input->post('id');
  if($this->form_validation->run() == true)
  {
    $select = $this->db->where('id' , $id)->get('tags')->row();
    if(!empty($select))
    {
